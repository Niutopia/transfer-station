...
页面中的“登录 Cookie 任务栏”用于维护目标站点的 Cloudflare 会话。系统会先使用提交 Cookie 时的浏览器 User-Agent 做隔离验证，验证成功后再成组替换本地配置，写入异常时回滚；验证失败不会覆盖现有凭证。Cookie 与 User-Agent 分别保存在忽略跟踪且权限为 `0600` 的 `data/auth-cookie.txt`、`data/auth-user-agent.txt`，不会写入状态快照、日志、镜像或 Git。
...
日常任务以 2 秒最小请求间隔运行，详情页串行解析并且只处理历史中未见的新条目；已见条目的补抓只能通过显式修复任务进行，避免短时间反复访问同一详情页。
...
抓取器会对 TLS 中断、超时、限流和临时服务错误执行指数退避重试。列表页采用有限并发，单页失败不会丢失其他榜单的结果；媒体链接超过 3 分钟或返回失效状态时，会在下载前自动重新解析并写回本地历史。若媒体域名命中未信任的 Clash Fake-IP，当前任务会重新解析签名地址并切换媒体线路，默认最多额外尝试 3 次；每轮仍执行完整公网地址校验，不会自动放宽私网或 Fake-IP 白名单。下载前还会用新签名链接短测直连与当前 HTTP 代理，自动选择更快的媒体路线；代理故障或较慢时安全回退直连。Docker 只注入专用 `MEDIA_HTTP_PROXY=http://host.docker.internal:7897`，不会让榜单/API 请求跟随全局代理。
如任何列表页在重试后仍失败，其余成功页面会继续处理，但 Web 面板会把本次任务标记为“部分完成”并显示失败页数。
...
- `data/ignored-media.json`：经用户确认永久跳过的无媒体条目；只保留记录，不会自动写入新失败，移除对应 key 即可恢复。
- `data/partials/`：下载中的临时 `.part` 分片。
- `data/logs/`：由每日任务生成的本地运行日志。
- `data/auth-cookie.txt`、`data/auth-user-agent.txt`：仅本机使用的登录会话与对应浏览器身份。
- `public/status.json`：供 Web 页面读取的只读状态快照。

监控器只在本地读取这些文件，不上传视频、Cookie、签名媒体地址或运行日志。
普通抓取和历史待处理项复核两种任务的成功下载都会进入同一份入库账本；忽略清单中的条目不会进入任一队列。
