...
抓取任务读取 `config/daily-sources.json`，默认抓取 5 个榜单的前 2 页。页面底部的“抓取链接任务栏”可以添加或删除 `/v.php` 榜单及 `/index.php` 首页链接，也可以删除全部来暂停抓取；无来源时 Web 不会启动空任务。修改会保存到该配置并在下一次手动任务中生效。任务运行期间配置会被锁定；删除链接不会删除已下载文件或历史记录。不同榜单及不同页之间统一按 `viewkey` 去重，再与跨日历史索引去重。
